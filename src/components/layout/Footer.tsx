      ...
      <div className="mx-auto max-w-7xl px-5 py-14 sm:px-10 lg:px-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-10 pb-12 border-b border-[#ECECEC]">
          ...
          <div className="flex flex-col gap-4">
            <Link href="/" className="flex flex-col gap-1 w-fit group">
              ...
              <span className="text-[8px] font-bold uppercase tracking-[0.22em] text-[#9B9B9B] leading-tight">
                The Notebook<br />of a Tech Woman
              </span>
            </Link>
          ...
          <div className="flex flex-col gap-3">
            <h3 className="text-[10px] font-sans font-bold uppercase tracking-[0.22em] text-foreground">Explore</h3>
            <ul className="flex flex-col gap-2.5">
              {exploreLinks.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-xs text-[#6B6B6B] hover:text-foreground transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          {/* Col 3: Information */}
          <div className="flex flex-col gap-3">
            <h3 className="text-[10px] font-sans font-bold uppercase tracking-[0.22em] text-foreground">Information</h3>
            <ul className="flex flex-col gap-2.5">
              {infoLinks.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="text-xs text-[#6B6B6B] hover:text-foreground transition-colors">
          ...
          {/* Col 4: Connect */}
          <div className="flex flex-col gap-3">
            <h3 className="text-[10px] font-sans font-bold uppercase tracking-[0.22em] text-foreground">Connect</h3>
            <ul className="flex flex-col gap-2.5">
              <li>
                <a
          ...
          <div className="flex flex-col gap-3">
            <h3 className="text-[10px] font-sans font-bold uppercase tracking-[0.22em] text-foreground">Stay in the loop</h3>
            ...
            <div className="min-h-[14px]">
              {state.success && (
                <span className="text-[10px] text-foreground font-medium animate-in fade-in">Subscribed!</span>
              )}
              {state.error && (
                <span className="text-[10px] text-destructive animate-in fade-in">{state.error}</span>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-8 text-xs text-[#9B9B9B]">
          ...
          <div className="flex gap-6">
            <Link href="/about" className="hover:text-foreground transition-colors">Terms of Use</Link>
