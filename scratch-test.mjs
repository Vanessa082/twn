...
async function runTests() {
  ...
  const { error: pageErr } = await supabase.from('shared_pages').insert({
    ...
    content: 'Today I realized that simple solutions are often the hardest to discover, but they are always the most rewarding to implement.',
    word_count: 21,
    status: 'pending'
  });
  ...
  const { data: article, error: artErr } = await adminClient.from('articles').insert({
    title: 'Test Article',
